...
class Encoder(nn.Module):
    ...
    #     self.fc = nn.Linear(calculated_flat_output, 1024)  # calculated_flat_output需要基于新的卷积配置计算
    #     self.fc_mean = nn.Linear(1024, 1024)  # 将维度从1024映射到潜在空间维度1024
    ...
    def __init__(self, input_channels):
        ...
        self.fc1 = nn.Linear(12288, 4096)  # Reducing dimension gradually
        self.fc2 = nn.Linear(4096, 1024)  # Further reduction to target latent size
        self.fc_mean = nn.Linear(1024, 1024)
        self.fc_log_var = nn.Linear(1024, 1024)

        self.sampling = Sampling()
    #     # Calculate flat output size by forwarding a dummy input
    #     self.feature_size = self._get_conv_output(input_channels)
    #     #self.fc = nn.Linear(self.feature_size, 1024)
    #     self.fc_mean = nn.Linear(self.feature_size, 1024)
    #     self.fc_log_var = nn.Linear(self.feature_size, 1024)
    #     self.sampling = Sampling()

    # def _get_conv_output(self, input_channels):
    #     # Create a dummy input tensor with the expected input shape (e.g., [1, input_channels, 501, 1501])
    #     dummy_input = torch.zeros(1, input_channels, 501, 1501)
    #     output = self.conv5(self.conv4(self.conv3(self.conv2(self.conv1(dummy_input)))))
    #     return int(torch.numel(output) / output.shape[0])  # Calculate total number of features in the output
    ...
    def forward(self, x):
        ...
        z_mean = F.relu(self.fc1(z_mean))
        z_log_var = F.relu(self.fc1(z_log_var))
        z_mean = self.fc2(z_mean)
        z_log_var = self.fc2(z_log_var)
...
class Decoder(nn.Module):
    ...
    #     self.reshape = lambda x: x.view(-1, 2, 8, 64)
    ...
    def __init__(self):
        super(Decoder, self).__init__()
        self.fc = nn.Linear(1024, 512 * 16 * 47)  # 将潜在空间维度映射回卷积展平前的维度
        self.conv_transpose1 = nn.ConvTranspose2d(512, 512, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        self.conv_transpose2 = nn.ConvTranspose2d(512, 256, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        self.conv_transpose3 = nn.ConvTranspose2d(256, 128, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        self.conv_transpose4 = nn.ConvTranspose2d(128, 64, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        #self.conv_transpose5 = nn.ConvTranspose2d(64, 64, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        
        # Separate paths for mu and log variance
        self.conv_transpose_x_mu = nn.ConvTranspose2d(64, 6, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
        self.conv_transpose_log_var = nn.ConvTranspose2d(64, 6, kernel_size=(3, 3), stride=(2, 2), padding=1, output_padding=1)
    ...
    def forward(self, z):
        x = self.fc(z)
        x = x.view(-1, 512, 16, 47)  # 重构到卷积层展平前的维度
        x = F.relu(self.conv_transpose1(x))
        print(f"Shape after conv_transpose1: {x.shape}")
        x = F.relu(self.conv_transpose2(x))
        print(f"Shape after conv_transpose2: {x.shape}")
        ...
        # print(f"Shape after conv_transpose5: {x.shape}")
        
        x_mu = self.conv_transpose_x_mu(x)
        print(f"Shape after conv_transpose_x_mu: {x_mu.shape}")
        ...
        x_mu = self.adaptive_pool(x_mu)
        x_log_var = self.adaptive_pool(x_log_var)
        print("Output mu shape:", x_mu.shape)
        print("Output log var shape:", x_log_var.shape)
        x_mu = x_mu.reshape(x_mu.shape[0], -1)
        x_log_var = x_log_var.reshape(x_log_var.shape[0], -1)
        x_mu_log_var = torch.cat([x_mu, x_log_var], dim=1)
        print(f"Final shape before return: {x_mu_log_var.shape}")
...
def train_vae(vae_model, train_loader, test_loader, optimizer, criterion, epochs):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in loop:
            ...
            total_kl += kl.item()
            reconstruction_loss = reconstruction(inputs, outputs[2])
...
def train_and_get_hidden_state(train_data, test_data, input_channels, init_model_path=None,save_model_path=None,save_file_name='feature1.pt'):
    ...
    if init_model_path==None:
        ...
        criterion_vae = kl_reconstruction_loss(0.005)
        train_vae(vae_model,vae_train_loader,vae_test_loader,optimizer_vae,criterion_vae,epochs=5)
...
def load_and_process_data(file_path):
    """加载单个数据文件，根据需要进行预处理，然后返回数据。"""
    data = np.load(file_path)
    # 在这里可以添加任何预处理步骤，例如标准化、重塑等
    return data
...
#     test_humidity = np.array(test_humidity)[:, 0, :, :]
    ...
    for train_file in training_files:
        train_data = load_and_process_data(os.path.join(base_dir, train_file))
        # 对 train_data 进行模型训练或其他处理

    # 测试数据处理可能与训练数据类似，但确保遵守正确的评估协议
    for test_file in testing_files:
        test_data = load_and_process_data(os.path.join(base_dir, test_file))
        # 使用 test_data 进行模型测试或验证
    ...
    # feature train
    train_and_get_hidden_state(train_data, test_data, 6, save_model_path='model_omega.pth', save_file_name='hidden_feature_omega.pt')
